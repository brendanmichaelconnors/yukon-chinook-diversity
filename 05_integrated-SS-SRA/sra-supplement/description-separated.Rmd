...
Recruitment abundances of adult Chinook ($R$) from 1985 to 2019 were treated as unobserved states and modeled as a function of spawner abundance ($S$), in brood year $y$, assuming a @ricker-1954 spawner-recruitment relationship with serially auto-correlated log-normal process variation:
...
where $\alpha$ is productivity (intrinsic rate of growth), $\beta$ is the magnitude of within brood-year density-dependent effects and $v$ reflects inter-annual variation in survival from egg to adulthood.
This variation was assumed to be correlated with lag-1 ($\phi$) over time:
...
where $\varepsilon_y$ reflects the portion of the survival anomaly $v_y$ that is temporally independent (white noise).
The first seven years of observed returns that were not linked to observations of spawner abundance in the spawner-recruitment relationship (eqn. \@ref(eq:ricker)) were modeled as random draws from a log-normal distribution with mean $\ln(R_0)$ and variance $\sigma_R^2$.
...
The number of Chinook salmon that returned to spawn in year $y$ and age $a$ ($a \in$ 4:7) was the product of the total return in year $y-a$ and the proportion of fish from brood year $y-a$ that returned at age $a$:

\begin{equation}
N_{y,a} = R_{y-a,a} p_{y-a,a}
(\#eq:apportion-R)
\end{equation}

where $p_{y,a}$ is the probability a fish spawned in brood year $y$ will mature at age $a$.
We modeled brood year variation in maturity schedules as Dirichlet random vectors drawn form a common hyperdistribution characterized by a mean maturation-at-age probability vector ($\boldsymbol{\pi}$) and an inverse dispersion parameter ($D$):
...
Lastly, harvest in a given year ($H$) was modeled as the product of total run size and the harvest rate ($U$) experienced:
